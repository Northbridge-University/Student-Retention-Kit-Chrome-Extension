        ...
        try {
            ...
            for (const suppFile of supplementaryFiles) {
                ...
                if (suppIsValid) {
                    ...
                    // or whose daysOut was corrupted/overwritten during the merge.
                    if (referenceDate) {
                        for (const student of students) {
                            const currentDaysOut = parseInt(student.daysOut);
                            const needsRecalc = isNaN(currentDaysOut) || student.daysOut === undefined || student.daysOut === null;
                            if (student.lda && needsRecalc) {
...
async function writeXlsxWithConditionalFormatting(wbBuffer, filename, condFmtSheets, tabColors = [], colorScale = {}, rowHighlights = [], cellValueFmts = [], tableMetadata = []) {
    ...
    for (const sheetIndex of sheetsToModify) {
        ...
        const COLOR_SCALE_PRESETS = {
            ...
            attendance: { minVal: 0,   midVal: 50,  maxVal: 100, low: 'FFF8696B', mid: 'FFFFEB84', high: 'FF63BE7B' }
        };
    ...
    for (let i = 0; i < tableMetadata.length; i++) {
        ...
        const colsXml = meta.columns.map((col, idx) => {
            const name = String(col.header)
                .replace(/&/g, '&amp;').replace(/</g, '&lt;')
                .replace(/>/g, '&gt;').replace(/"/g, '&quot;');
            // Add totalsRowFunction if provided for this column
            const totalsFunc = hasTotals && meta.totalsRowFunctions[idx]
                ? ` totalsRowFunction="${meta.totalsRowFunctions[idx]}"`
                : (hasTotals ? ' totalsRowFunction="none"' : '');
            const totalsLabel = hasTotals && meta.totalsRowLabels && meta.totalsRowLabels[idx]
                ? ` totalsRowLabel="${meta.totalsRowLabels[idx]}"`
                : '';
            return `<tableColumn id="${idx + 1}" name="${name}"${totalsFunc}${totalsLabel}/>`;
        }).join('');
